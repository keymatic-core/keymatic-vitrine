...
export default function EcomHero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      {/* Ice/gelo background — matches vitrine section-light */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 w-full pt-28 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          {/* Text */}
          <div>
            {/* Badge — dark style with border */}
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4 }}
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-orange-500/25 bg-orange-500/5 backdrop-blur-sm text-[13px] text-orange-600 font-normal mb-8"
            >
            ...
            {/* Trust badges — orange icons, darker text */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              {TRUST_BADGES.map((badge) => {
                ...
                return (
                  <div
                    key={badge.label}
                    className="flex items-center gap-1.5 text-[12px] text-zinc-600"
                  >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#f4f4f5] to-transparent" />
    </section>
  );
}
